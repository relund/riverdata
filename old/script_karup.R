...
# REMEMBER TO SET WD TO FILE LOC
...
dat3 <- read.table(file="data_karup_fangster_alle_2003-2016.csv", dec=",", sep=";", header = TRUE, stringsAsFactors = FALSE)
...
dat3 <- bind_rows(dat3, dat31) %>% arrange(Date, Name)
...
dat4 <- dat4 %>% distinct(Date, Name, Length, Weight, Place, .keep_all = T)
dub <- dat4 %>% group_by(Date, Name, Length, Place) %>% dplyr::filter(n()>1)
View(dub)
