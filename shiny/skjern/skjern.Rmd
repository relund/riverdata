...
datMarkers <- read_csv(paste0(prefix, "data_skjern_mapmarkers.csv"), col_types = "fccddff") %>% 
  ...
  mutate(Desc = map(Desc, HTML), Icon = str_c("www/", Icon)) %>% 
  select(-Text) 
...
# infoIcons <- icons(str_c("www/", datMarkers %>% pull(Icon)),
...
groups <- NULL
# Markers
maplet <- mapAddMarkers(maplet, "Stednavne", 
                     datMarkers %>% 
                       filter(str_detect(Group, regex("Standpladser|Sten|Spang|Bro", ignore_case = T))))
maplet <- mapAddMarkers(maplet, "Parkering", 
                        datMarkers %>% 
