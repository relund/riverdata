...
datStatYearSalmon <- yearlyStat(datCatchSalmon)
...
```{r Catches table, context = "server"}
...
```{r, context = "server"}
  output$dTSeaCatch <- DT::renderDataTable(
    DT::datatable(
    datCatchTrout[,c(Date:Method, Fulton, Misc, NoWeight)],
    class = 'stripe row-border order-column nowrap compact', # nowrap display compact
    rownames = FALSE,
    filter = 'bottom',
    escape = FALSE,
...
### 
```{r}
sliderInput("iInvYear", label = "Vælg årstal:",
            min = 2004, max = year(now()), sep = "",
            value = c(2016, year(now())), step = 1)
sliderInput("iInvDay", label = "Vælg dato interval: ",
...
selectInput("iXUnit", "Vælg enhed på x-aksen:", cIXUnit, selected = "Year")
cIYUnit <- c("Fisk fanget" = "yVTotal",
             ...
             "Max længde" = "yVMaxL",
             "Total vægt" = "yVTotalW")
selectInput("iYUnit", "Vælg enhed på y-aksen:", cIYUnit)
```
...
selectInput("iData", "Vælg datasæt:", cIData, "All")
cIGroup <- c("Område" = "Place",
            "Metode" = "Method",
            "Køn" = "Sex",
            "Hjemtaget" = "Killed",
            "Ingen" = "None")
selectInput("iGroup", "Vælg gruppe inddeling:", cIGroup, "None")
```
...
if (input$iData == "KilledTrue") dat <- dat %>% dplyr::filter(Killed)
if (input$iData == "KilledFalse") dat <- dat %>% dplyr::filter(!Killed)
if (input$iData == "WeightTrue") dat <- dat %>% dplyr::filter(NoWeight == 0)
dat <- dat %>% 
