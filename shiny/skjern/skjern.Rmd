...
datCatchSalmon <- read_csv(paste0(prefix, "data_skjern_catch_salmon.csv")) %>% 
  mutate(
    ...
    Month = month(Date, label = T), Week = week(Date), Year = year(Date), NoWeight = is.na(Weight)
  )
...
DT::renderDataTable(
  ...
  formatStyle('Weight', 'NoWeight', color = styleEqual(c(TRUE, FALSE), c("black","grey"))) %>% 
...
```{r}
  ...
  formatDate('Date', 'toLocaleDateString') %>%
  formatRound(c('Length','Weight','Fulton'), digits = c(0,1,2)) %>%
  formatStyle('Weight', 'NoWeight', color = styleEqual(c(TRUE, FALSE), c("black","grey")))
)
```
