...
# Water levels 
datWLevel <- 
  read_csv(paste0(prefix, "data_skjern_waterlevel_relative_long_", year(now()), ".csv")) %>% 
  dplyr::filter(Date > now() - days(14)) %>% 
  dplyr::filter(str_detect(Place, "Skjern|Rind")) %>% 
  mutate(Level = Level*100, LevelRelative = 100*LevelRelative)
...
output$cSalYear <- renderValueBox({
  total <- datCatchSal %>% dplyr::filter(year(Date) == year(now())) %>% nrow()
  killed <- datCatchSal %>% dplyr::filter(year(Date) == year(now()), Killed == TRUE) %>% nrow()
...
output$cSalBig <- renderValueBox({
  ct <- datCatchSal %>% dplyr::filter(year(Date) == year(now())) %>% 
...
selectInput("iPlaceW", "Sted:", cIPlaceWUnit, selected = cIPlaceWUnit[2])
```

###
```{r}
cIScaleWUnit <- c("Relativ" = "LevelRelative", "Absolut" = "Level")
selectInput("iScaleW", "Vælg enhed på y-aksen:", cIScaleWUnit)
```
...
output$plotWLevelPresent <- renderPlotly({
  ...
  p <- ggplot(
    data = datWLevel %>% 
      ...
      transmute(Dato = Date, Vandstand = .data[[input$iScaleW]]), 
    aes(x = Dato, y = Vandstand, group = 1, # must be added for working https://stackoverflow.com/questions/45948926/ggplotly-text-aesthetic-causing-geom-line-to-not-display
        text = str_c("Dato: ", format(Dato, "%e. %b %R"), '<br>Vandstand: ', round(Vandstand,2)))) + 
    ...
    ylab(yLab) + xlab("Dato") +
    #theme(legend.position="bottom") +
    scale_x_datetime(date_breaks = "1 day", date_labels = "%e. %b") 
  if (input$iScaleW == "LevelRelative") p <- p + geom_hline(yintercept = 0, colour="black")
  ggplotly(p, tooltip = c("text")) %>% plotly::layout(legend = list(orientation = "h", x = 0.3, y = -0.4))
})
```
...
```{r}
plotlyOutput("plotWLevelYear")
```

```{r Vandstand årlig, context = "server"}
# renderPlot({
...
# })
output$plotWLevelYear <- renderPlotly({
  loc <- Sys.setlocale("LC_TIME", "Danish")
  on.exit(Sys.setlocale("LC_TIME", loc))
  readGHWLevels <- function(years) {
    dat <- NULL
    for (y in years) {
  ...
  p <- ggplot(data = datWLevelAllL, aes(x = Date, y = Level, group = 1, 
        text = str_c("Dato: ", format(Date, "%e. %b %R"), '<br>Vandstand: ', round(Level,2)))) +
    geom_line(aes(), size = 0.25, na.rm = T) +
    facet_grid(rows = vars(Place), scales = "free") +
    theme(legend.position="bottom") +
    ylab("Relativ vandstand (m)") + xlab("Dato") 
  ggplotly(p, tooltip = c("text")) %>% plotly::layout(legend = list(orientation = "h", x = 0.3, y = -0.4))
...
Statistik `r curYear` {data-navmenu="Laks"}
=======================================================================
...
selectInput("iXUnitSal", "Vælg enhed på x-aksen:", cIXUnit, selected = "Year")
cIYUnit <- c("Fisk fanget" = "yVTotal",
             ...
             "Max længde" = "yVMaxL",
             "Total vægt" = "yVTotalW")
selectInput("iYUnitSal", "Vælg enhed på y-aksen:", cIYUnit)
```
...
selectInput("iDataSal", "Vælg datasæt:", cIData, "All")
cIGroup <- c("Område" = "Place",
            "Metode" = "Method",
            "Køn" = "Sex",
            "Hjemtaget" = "Killed",
            "Ingen" = "None")
selectInput("iGroupSal", "Vælg gruppe inddeling:", cIGroup, "None")
```
...
```{r}
cIXUnit <- c("Dag" = "Day", "Uge" = "Week", "Måned" = "Month", "År" = "Year")
selectInput("iXUnitSea", "Vælg enhed på x-aksen:", cIXUnit, selected = "Year")
cIYUnit <- c("Fisk fanget" = "yVTotal",
             "Gennemsnitlig vægt" = "yVAvgW",
             "Gennemsnitlig længde" = "yVAvgL",
             "Max vægt" = "yVMaxW",
             "Max længde" = "yVMaxL",
             "Total vægt" = "yVTotalW")
selectInput("iYUnitSea", "Vælg enhed på y-aksen:", cIYUnit)
```
...
cIData <- c("Alle fangster" = "All",
            ...
            "Genudsatte fisk" = "KilledFalse",
            "Fangster med vægt angivelse" = "WeightTrue")
selectInput("iDataSea", "Vælg datasæt:", cIData, "All")
cIGroup <- c("Område" = "Place",
            "Metode" = "Method",
            "Køn" = "Sex",
            "Hjemtaget" = "Killed",
            "Ingen" = "None")
selectInput("iGroupSea", "Vælg gruppe inddeling:", cIGroup, "None")
```
...
```{r}
plotOutput("plotLock14")
```

```{r, context = "server"}
output$plotLock14 <- renderPlot({
  ggplot(datFlow14) + 
