...
### Relativ vandstand i forhold til gennemsnit
...
output$plotWLevelPresent <- renderPlotly({
  ...
  datS <- datWLevel %>% 
    dplyr::filter(Date > now() - days(14)) %>% 
    dplyr::filter(str_detect(Place, "Skjern|Rind"))
  p <- ggplot(data = datS, aes(x = Date, y = Level, group = 1, # must be added for working https://stackoverflow.com/questions/45948926/ggplotly-text-aesthetic-causing-geom-line-to-not-display
