...
# Catch - Records 
datCatchSea <- readCatch(prefix, datWeightSea)
...
# Water levels 
datWLevel <- 
  read_csv(paste0(prefix, "data_karup_waterlevel_web.csv"), col_types = "Tffdd") 

# Water temperature
datWTemp <- 
  read_csv(paste0(prefix, "data_karup_watertemp_web.csv"), col_types = "Tffd") 

# Maps data
datMarkers <- read_csv(paste0(prefix, "data_karup_mapmarkers.csv"), col_types = "ffddf") 
datLines <- read_csv(paste0(prefix, "data_karup_maplines.csv"), col_types = "ffddif") 

placeIcon <- makeIcon("www/place.png", iconWidth = 32, iconHeight = 37, iconAnchorX = 16, iconAnchorY = 18) 
parkIcon <- makeIcon("www/park.png", iconWidth = 32, iconHeight = 37, iconAnchorX = 16, iconAnchorY = 18) 
...
maplet <- leaflet() %>% 
  ...
  setView(9.016712672451805, 56.40970340006212,  zoom = 10) 
# Markers
maplet <- maplet %>% 
  addMarkers(~long, ~lat, popup = ~Desc, label = ~Desc,
             icon = placeIcon, group = "Stednavne",
             clusterOptions = markerClusterOptions(
                 showCoverageOnHover = FALSE),
             data = datMarkers %>% filter(Group == "Stednavne")) %>%
  addMarkers(~long, ~lat, popup = ~Desc, label = ~Desc,
             icon = parkIcon, group = "Parkering",
             clusterOptions = markerClusterOptions(
                 showCoverageOnHover = FALSE),
             data = datMarkers %>% filter(Group == "Parkering")) 
# Lines Parkering
dat <- datLines %>% filter(Group == "Parkering") %>% group_by(LineGroupId, Desc) %>% nest()
map(1:nrow(dat), function(i) {
  maplet <<- maplet %>% 
    addPolylines(~long, ~lat, popup = dat$Desc[i], label = dat$Desc[i],
                 group = "Parkering", color = "#eb9834", weight = 2, opacity = 0.75,
                 data = dat[i,] %>% unnest(col = c(data)))
  return(invisible(NULL))
})
# Lines Fiskevand
dat <- datLines %>% filter(Group == "Fiskevand") %>% group_by(LineGroupId, Desc, Club) %>% nest()
map(1:nrow(dat), function(i) {
  maplet <<- maplet %>% 
    addPolylines(~long, ~lat, popup = dat$Desc[i], label = dat$Desc[i],
                 group = str_c(dat$Club[i], " (fiskevand)"), color = "#3C8AE6", weight = 2, opacity = 0.9,
                 data = dat[i,] %>% unnest(col = c(data)))
  return(invisible(NULL))
})
# Lines Medlemmer
dat <- datLines %>% filter(Group == "Medlemmer") %>% group_by(LineGroupId, Desc, Club) %>% nest()
map(1:nrow(dat), function(i) {
  maplet <<- maplet %>% 
    addPolylines(~long, ~lat, popup = dat$Desc[i], label = dat$Desc[i],
                 group = str_c(dat$Club[i], " (medlem)"), color = "#EBE053", weight = 2, opacity = 0.75,
                 data = dat[i,] %>% unnest(col = c(data)))
  return(invisible(NULL))
})
# Lines Dagkort
dat <- datLines %>% filter(Group == "Dagkort") %>% group_by(LineGroupId, Desc, Club) %>% nest()
map(1:nrow(dat), function(i) {
  maplet <<- maplet %>% 
    addPolylines(~long, ~lat, popup = dat$Desc[i], label = dat$Desc[i],
                 group = str_c(dat$Club[i], " (dagkort)"), color = "#000000", weight = 2, opacity = 0.75,
                 data = dat[i,] %>% unnest(col = c(data)))
  return(invisible(NULL))
})
# Lines Gæstekort
# dat <- datLines %>% filter(Group == "Gæstekort") %>% select(Line) %>% unnest(col = c(Line))
# map(1:nrow(dat), function(i) {
#   maplet <<- maplet %>% 
#     addPolylines(~long, ~lat, popup = dat$Desc[i], label = dat$Desc[i],
#                  group = "Gæstekort", color = "#000000", weight = 2, opacity = 0.75,
#                  data = dat[i,] %>% unnest(col = c(LineCord)))
# })
...
maplet <- maplet %>% 
  ...
  addLayersControl(
    ...
    overlayGroups = c("Stednavne", "Parkering", "HI-LF (medlem)", "HI-LF (dagkort)", "HI-LF (fiskevand)"),
    options = layersControlOptions(collapsed = FALSE)
  ) %>% 
  hideGroup(c("Stednavne", "Parkering"))
```
...
output$cSeaYear <- renderValueBox({
  total <- datCatchSea %>% dplyr::filter(year(Date) == year(now())) %>% nrow()
  killed <- datCatchSea %>% dplyr::filter(year(Date) == year(now()), Killed == TRUE) %>% nrow()
...
output$cSeaBig <- renderValueBox({
  ct <- datCatchSea %>% dplyr::filter(year(Date) == year(now())) %>% 
    dplyr::filter(Weight == max(Weight, na.rm = T)) %>% arrange(Weight, Length)
  val <- if_else(nrow(ct) > 0, paste(ct$Weight[1], "kg og", ct$Length[1], "cm"), "-")
  cap <- if_else(nrow(ct) > 0,paste("Største fisk i år fanget af", ct$Name[1]), "Største fisk i år")  
  valueBox(
    caption = cap,
    value = val,
    icon = "fa-trophy",
...
selectInput("iPlaceW", "Sted:", cIPlaceWUnit, selected = cIPlaceWUnit[2], selectize = FALSE)
```
...
Statistik `r curYear` {data-navmenu="Havørred"}
=======================================================================
...
output$plotCatchSeaMonthNow <- renderPlot({
  ggplot(data = datCatchSea %>% dplyr::filter(Date > ymd(paste0(curYear,"-01-01"))), 
       mapping = aes(x = Month)) + 
...
* [Yr Karup](https://www.yr.no/sted/Danmark/Midtjylland/Karup/){target="_blank"}
* [DMI Karup](https://www.dmi.dk/lokation/show/DK/2619068/Karup/){target="_blank"}
* [Yr Hagebro](https://www.yr.no/sted/Danmark/Midtjylland/Haderup/){target="_blank"}
* [DMI Hagebro](https://www.dmi.dk/lokation/show/DK/2620960/Haderup/){target="_blank"}
* [Yr Vridsted](https://www.yr.no/sted/Danmark/Midtjylland/Vridsted/){target="_blank"}
* [DMI Vridsted](https://www.dmi.dk/lokation/show/DK/2609962/Vridsted/){target="_blank"}
* [Yr Skive](https://www.yr.no/sted/Danmark/Midtjylland/Skive/){target="_blank"}
