...
# infoIcons <- icons(str_c("www/", datMarkers %>% filter(Group == "Info") %>% pull(Icon)),
...
#   addMarkers(~long, ~lat, label = ~Desc,
#              icon = infoIcons, group = "Parkering",
#              clusterOptions = markerClusterOptions(
#                  showCoverageOnHover = FALSE),
#              data = datMarkers %>% filter(Group == "Info")) 
# # Lines Parkering 
# dat <- datLines %>% filter(Group == "Parkering") %>% group_by(LineGroupId, Desc, Text) %>% nest()
# map(1:nrow(dat), function(i) {
#   maplet <<- maplet %>% 
#     addPolylines(~long, ~lat, label = ~Desc,
...
Vægten for genudsatte fisk er [estimeret](#estimat-af-vægt) udfra tidligere fangster og givet i grå farve. 
