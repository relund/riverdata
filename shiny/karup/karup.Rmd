...
# Map places
datPlaces <- read_xml("https://www.google.com/maps/d/u/0/kml?mid=1XJoAUKY_-kbmhZgovPpLgi82Gn8&forcekml=1")
xml_ns_strip(datPlaces)
datPlaces <- xml_find_all(datPlaces, "//Placemark")
datPlaces <- tibble(name = xml_text(xml_find_all(datPlaces, ".//name")), 
            cord = xml_text(xml_find_all(datPlaces, ".//coordinates"), trim = TRUE) ) %>% 
...
output$map <- renderLeaflet({
  leaflet(data = datPlaces) %>% 
    # Base groups
    addTiles(group = "Kort") %>%
    addProviderTiles('Esri.WorldImagery', group = "Luftfoto") %>% 
    addProviderTiles("CartoDB.PositronOnlyLabels", group = "Luftfoto") %>% 
    setView(9.016712672451805, 56.40970340006212,  zoom = 14) %>% 
    # Overlay groups
    # add places 
    addMarkers(~long, ~lat, popup = ~name, label = ~name,
               icon = placeIcon, 
               group = "Stednavne") %>% 
    # Layer control
    addLayersControl(
      baseGroups = c("Luftfoto", "Kort"),
      overlayGroups = c("Stednavne"),
