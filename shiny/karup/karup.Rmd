...
# Water temperature
datWTemp <- 
  read_csv(paste0(prefix, "data_karup_watertemp_web.csv"), col_types = "Tffdd") 
```
...
output$plotWTempPresent <- renderPlotly({
  ...
  on.exit(Sys.setlocale("LC_TIME", loc))
  tmp <- datWTemp %>% 
      transmute(Date, Temp, Avg, Year = YGroup, Size = if_else(Year == year(now()), 2, 1))
  p <- ggplot(data = tmp,
              aes(x = Date, y = Temp, color = Year, size = Size, group = 1, 
                  text = str_c("Dato: ", format(Date, "%e. %b %H:00"), '<br>Temp: ', Temp, ''))) +
    geom_smooth(mapping = aes(y = Avg), data = datWTemp, colour="black", size = 0.1, span = 0.5) +
    geom_line(na.rm = T) + 
