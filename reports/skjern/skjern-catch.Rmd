...
```{r}
snip_plot_catch(datCatchSal, .unit = "days", .look_back = 30)
  # dat <- datCatchSal %>%
...
```{r}
snip_plot_catch(datCatchSal, .unit = "weeks", .look_back = 30)
  # dat <- datCatchSal %>% 
...
```{r}
snip_plot_catch(datCatchSal, .unit = "months", .look_back = 12)
  # dat <- datCatchSal %>% 
  #   dplyr::filter(Date > ymd(paste0(curYear,"-01-01"))) %>% 
  #   select(Month, Place)
  # 
...
```{r}
snip_plot_catch(datCatchSea, .unit = "days", .look_back = 30)
  # dat <- datCatchSea %>% 
  #   dplyr::filter(Date > now(tzone = "CET") - days(30)) %>% 
  #   select(Date, Place) %>% 
  #   arrange(desc(Date))
...
```{r}
snip_plot_catch(datCatchSea, .unit = "weeks", .look_back = 30)
  # dat <- datCatchSea %>% 
  #   dplyr::filter(Date > ymd(paste0(curYear,"-01-01"))) %>% 
  #   select(Week, Place)
  # 
...
```{r}
snip_plot_catch(datCatchSea, .unit = "months", .look_back = 12)
  # dat <- datCatchSea %>% 
  #   dplyr::filter(Date > ymd(paste0(curYear,"-01-01"))) %>% 
  #   select(Month, Place)
  # 
