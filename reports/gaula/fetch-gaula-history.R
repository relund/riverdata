...
river_id <- 25
station_id <- "122.9.0"
temperature_station_id <- "122.11.0"
...
report_dir <- if (file.exists(file.path(cwd, "gaula.html"))) {
  cwd
} else {
  normalizePath(file.path(cwd, "reports", "gaula"), mustWork = FALSE)
}
repo_dir <- if (basename(report_dir) == "gaula" && basename(dirname(report_dir)) == "reports") {
  dirname(dirname(report_dir))
} else {
  cwd
}
output_file <- file.path(repo_dir, "data", "gaula-history.json")

neutral_abs_cms <- 1
neutral_rel <- 0.03
neutral_abs_stage_m <- 0.05
neutral_stage_rel <- 0
release_flag_schema_version <- 3
...
fetch_measurement_for_year <- function(year, parameter, value_name, station = station_id) {
  message("Fetching ", value_name, " for ", year, " at station ", station)
  ...
  if (httr::status_code(response) == 404) {
    message("No ", value_name, " measurements for ", year, " at station ", station)
...
fetched_daily_temperature <- map_dfr(years_to_fetch, ~ fetch_measurement_for_year(.x, 1003, "water temperature", temperature_station_id)) %>%
  group_by(date) %>%
...
payload <- list(
  ...
  temperature_station_id = temperature_station_id,
  years = years,
  species_filter = "Laks",
