...
```{r, echo = TRUE}
for (i in 3:8) testM(i,i+1)
```

Model 8 and 9 seems to be best.

```{r}
testM(8,9)
```

Note the only difference are which group August is added to. In August the are both new and old fish in the river.
